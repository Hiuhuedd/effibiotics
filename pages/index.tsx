...
const Home: NextPage = () => {
   ...
   const isAdmin = router.query.isAdmin === 'true';

   useEffect(() => {
    if (isAdmin) {
      // alert(`"This is Admin account: allows you access the scrapper settings"`)
    }else{
      // alert(`"This is user:"`)

    }
    if (router)     router.push({pathname:'/domains',query: { isAdmin: isAdmin.toString() } });
